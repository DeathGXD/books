...
#### max.request.size  
这个参数控制了生产者发送的最大请求量。它涵盖了生产者可以发送的最大消息量和在一个次请求中生产者可以发送的消息数量。打个比方，默认最大的请求量为1MB，那么可以发送的最大的消息是1MB，或者说生产者一次请求中可以批量发送1000条1K的消息。除此之外，broker也有它自己的限制它接受的最大的消息量的方法(message.max.bytes)。通常这两个值相同是非常不错的选择，那么生产者将不用尝试发送一个将被broker拒绝的消息量。  
...
```  
使用CustomerSerializer配置生产者将允许你去定义ProducerRecord<String, Customer>，并且直接发送Customer数据和传递Customer对象到生产者。这个例子相当简单，但是你也能看出代码相当脆弱。如果我们
